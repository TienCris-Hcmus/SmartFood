...
//	}
	...
	public List<Food_Store_Model> findRandom() {
		StringBuilder sql = new StringBuilder("SELECT f.IDFood, f.Name_, f.Price, f.Image, s.StoreName FROM food f, store s WHERE f.IDStore = s.IDStore order by RAND() limit 1");
		return query(sql.toString(), new Food_Store_Mapper());
	}
